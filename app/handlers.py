...
async def execute_tasks_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    if len(ls_channel) == 0:
        ...
        if aval_tasks == 'yes':
            ...
            await message.answer('‼️У вас пока не создан план дня, пожалуйста создайте его, нажав на кнопку Редактировать дневной план')
    else:
...
async def daily_statics_handler(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if len(ls_channel) == 0:
        ...
        if aval_tasks == 'yes':
            ...
            await callback.message.answer('‼️У вас не создан дневной план, пожалуйста создайте его, нажав на кнопку Редактировать дневной план')
    else:
        sub_mes = '💥Чтобы использовать данную функцию, вы должны быть подписаны на эти каналы:\n'
        for i in range(len(ls_channel)):
            sub_mes += f'{i+1} {ls_channel[i]}\n'
